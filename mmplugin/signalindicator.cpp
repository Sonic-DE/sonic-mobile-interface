...
{
    connect(ModemManager::notifier(), &ModemManager::Notifier::modemAdded, this, [=]() {
        updateModem();
    });
    connect(ModemManager::notifier(), &ModemManager::Notifier::modemRemoved, this, [=]() {
        updateModem();
    });
}
...
{
    return ModemManager::modemDevices().size() > 0;
}
