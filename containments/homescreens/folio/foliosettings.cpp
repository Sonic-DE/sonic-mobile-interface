...
const QString CFG_GROUP_GENERAL = QStringLiteral("General");
...
KConfigGroup FolioSettings::generalConfigGroup() const
{
    ...
    return m_homeScreen->config().group(CFG_GROUP_GENERAL);
}
...
void FolioSettings::migrateConfigFromPlasma6_4()
{
    ...
    auto migrateStr = [&oldConfigGroup, &generalGroup](const QString &newKey, const QString &oldKey, const QString &oldDefaultValue) {
    ...
    auto migrateInt = [&oldConfigGroup, &generalGroup](const QString &newKey, const QString &oldKey, int oldDefaultValue) {
        if (!oldConfigGroup.hasKey(oldKey) || generalGroup.hasKey(newKey)) {
            return;
        }
        generalGroup.writeEntry(newKey, oldConfigGroup.readEntry(oldKey, oldDefaultValue));
    ...
    migrateStr(CFG_KEY_FAVORITES, u"Favourites"_s, u"{}"_s);
    migrateStr(CFG_KEY_PAGES, u"Pages"_s, u"[[]]"_s);
    migrateInt(CFG_KEY_HOMESCREEN_ROWS, u"homeScreenRows"_s, 5);
    migrateInt(CFG_KEY_HOMESCREEN_COLS, u"homeScreenColumns"_s, 4);
    migrateInt(CFG_KEY_SHOW_PAGES_APPLABELS, u"showPagesAppLabels"_s, true);
    migrateInt(CFG_KEY_SHOW_FAVORITES_APPLABELS, u"showFavoritesAppLabels"_s, false);
    migrateInt(CFG_KEY_LOCK_LAYOUT, u"lockLayout"_s, false);
    migrateInt(CFG_KEY_DELEGATE_ICON_SIZE, u"delegateIconSize"_s, 48);
    migrateInt(CFG_KEY_SHOW_FAVORITES_BAR_BACKGROUND, u"showFavoritesBarBackground"_s, true);
    migrateInt(CFG_KEY_PAGE_TRANSITION_EFFECT, u"pageTransitionEffect"_s, (int)SlideTransition);
    migrateInt(CFG_KEY_SHOW_WALLPAPER_BLUR, u"wallpaperBlurEffect"_s, (int)Full);
