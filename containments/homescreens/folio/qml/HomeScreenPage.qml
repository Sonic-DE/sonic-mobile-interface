...
Item {
    ...
    function findFirstDelegate() {
        ...
        for (let i = 0; i < delegateRepeater.count; ++i) {
            ...
            if (delegate.row < firstDelegate.row
                || (delegate.column < firstDelegate.column && delegate.row === firstDelegate.row)) {
        ...
        while (x >= 0 && x < folio.HomeScreenState.pageColumns && y >= 0 && y < folio.HomeScreenState.pageRows) {
            for (let i = 0; i < delegateRepeater.count; ++i) {
                ...
                if (delegate.row === y && delegate.column === x
                    && (delegate.pageDelegate.type === Folio.FolioDelegate.Application || delegate.pageDelegate.type === Folio.FolioDelegate.Folder)) {
                    return delegate;
                }
            }
            x += dx;
