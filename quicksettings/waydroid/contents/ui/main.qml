...
QS.QuickSetting {
    ...
    Connections {
        ...
        function onStatusChanged() {
            console.log("QS Waydroid Status Changed", AIP.WaydroidDBusClient.status, AIP.WaydroidDBusClient.Initialized)
        }
    }

    function toggle(): void {
        if (AIP.WaydroidDBusClient.sessionStatus === AIP.WaydroidDBusClient.SessionRunning) {
            AIP.WaydroidDBusClient.stopSessionQml()
        } else {
            AIP.WaydroidDBusClient.startSessionQml()
        }
    }
}
