...
void WaydroidDBusObject::refreshApplications()
{
    if (m_sessionStatus != SessionRunning) {
        ...
        for (auto &appObject : m_applicationObjects) {
    ...
    const QList<WaydroidApplicationDBusObject> newApplications = WaydroidApplicationDBusObject::parseApplicationsFromWaydroidLog(inFile);
    ...
    QList<WaydroidApplicationDBusObject> toInsert;

    // Check which applications need to be added or are already present
    for (const WaydroidApplicationDBusObject &application : newApplications) {
    ...
    for (const WaydroidApplicationDBusObject &application : toInsert) {
        WaydroidApplicationDBusObject appObject(application.name(), application.packageName(), this);
        appObject.registerObject();
        m_applicationObjects.append(std::move(appObject));
    }
}
