...
void WaydroidState::refreshSessionInfo()
{
    ...
    const QRegularExpressionMatch ipAddressMatch = ipAdressRegExp.match(output);
    if (ipAddressMatch.hasMatch() && ipAddressMatch.lastCapturedIndex() > 0) {
        m_ipAddress = ipAddressMatch.captured(ipAddressMatch.lastCapturedIndex());
    } else {
        m_ipAddress = "";
    }
